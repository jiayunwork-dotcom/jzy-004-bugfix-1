...
服务把校验码按该档的字节序约定追加到数据后（非反转档高位字节在前，反转档低位字节在前），重新走一遍除法，余数等于该档约定的固定校验值（residue）时判为通过：
...
| `invalid_params` | 显式参数越界、model/params 同时给或都不给、校验码超出位宽 |
...
- **反转语义只定义一次**：`refin`（输入字节逐比特反转）与 `refout`（输出寄存器按全宽反转）只在 `internal/crc` 核心包中实现，编码与校验两侧、按位与查表两条路径共用同一处定义，不存在两侧不一致的可能。
